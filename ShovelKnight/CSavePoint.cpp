	...
	, m_bNoFirst(false)
{
	...
	m_pAnim->AddAnimation(L"Flame", m_pTex, RECT{ 0,(int)m_tSize.y * 0,(int)m_tSize.x,(int)m_tSize.y }, 5, 0.2f);
	m_pAnim->AddAnimation(L"Flaming", m_pTex, RECT{ 0,(int)m_tSize.y * 1,(int)m_tSize.x,(int)m_tSize.y }, 3, 0.2f);
}
...
DIR CSavePoint::OnCollisionEnter(CCollider * _other)
{
